...
def main():
    ...
    if args.do_train:
        ...
        test_examples, test_label_list, test_label_list2, test_hypo_seen_str_indicator, test_hypo_2_type_index = processor.get_examples_title_test(
            './title/test_new_entity.txt', seen_types, True, 50000)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            test_probs = []
            print(pred_probs)
            with open('test_inference_title.txt', 'w') as my_file1:
                for i in range(0, len(pred_probs), 3):
                    ...
                    my_file1.write(str(test_examples[i].text_b.split('is')[0] + ' ' + test_examples[i].text_a)+' '+str(dic[my_index])+' '+str(my_list)+'\n')
            ...
            seen_acc, unseen_acc = evaluate_emotion_zeroshot_TwpPhasePred(pred_probs, pred_binary_labels_harsh, pred_binary_labels_loose, test_label_list2, test_hypo_seen_str_indicator, test_hypo_2_type_index, seen_types)
            ...
            test_f1_list.append(unseen_acc)
            test_loss_list.append(test_tr_loss)
